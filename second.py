...
#st.set_page_config(layout="wide", page_icon=":computer:", page_title="IT Computer Service")
    ...
    #form = col1.form("service_form",clear_on_submit=True)
    client = col1.text_input("Client name")
    dept = col1.selectbox("Department Name",["IT","Admin","Committee"],index=0)
    model =col1.selectbox("Computer Model",["2010","2011","2012"],index=0)
    repair = col1.selectbox("Repair Time",["1","2","3","4","5"],index=0)
    brand = col1.selectbox("Computer Brand Name",["Dell","Acer","Apple"],index=0)
    error = col1.selectbox(
        "Error Type",
        ["Window Error",
        "UPS Error",
        "Hard Drive failure",
    ...
    submit = col1.form_submit_button("Generate PDF")

    col2.write("Staff Information:")
    form2 = col2.form("staff_form")
    name = form2.text_input("Staff name")
    time = form2.text_input("Service Time")
    service = form2.text_input("Finish Date")
    incharge = form2.selectbox("Incharge Name",["Ei","Su","Khaing"],index=0)
    remote = form2.text_input("Anydesk ID")
    grade = form.slider("Grade", 1, 100, 60)
    contact = form2.text_input("ဆက်သွယ်ရမည့်ဖုန်းနံပါတ်")

    submit2 = form2.form_submit_button("Save Information")
